import type { ApiContextProps } from '@mono/server/src/context'
import {
  Apple,
  Discord,
  GitHub,
  ...
  if (!providers[name]) {
    if (name === 'apple') {
      providers[name] = new Apple(
        ...
        new TextEncoder().encode(ctx.env.APPLE_CERTIFICATE),
        `${origin}/oauth/${name}`
      )
    }
    if (name === 'discord') {
    ...
    if (name === 'github') {
      providers[name] = new GitHub(ctx.env.GITHUB_CLIENT_ID, ctx.env.GITHUB_CLIENT_SECRET, `${origin}/oauth/${name}`)
    }
    if (name === 'google') {
...
export const getAuthorizationUrl = async (ctx: ApiContextProps, service: AuthProviderName) => {
  ...
  const secure = ctx.c?.req.url.startsWith('https:') ? 'Secure; ' : ''
  ...
  return await match({ provider, service })
    .with({ service: 'google', provider: P.instanceOf(Google) }, async ({ provider }) => {
      ...
      const url = await provider.createAuthorizationURL(state, codeVerifier, ['https://www.googleapis.com/auth/userinfo.email'])
    ...
    .with({ service: 'apple', provider: P.instanceOf(Apple) }, async ({ provider }) => {
      const url = await provider.createAuthorizationURL(state, ['email'])
      url.searchParams.set('response_mode', 'form_post')
      return url
    })
    .with({ service: 'discord', provider: P.instanceOf(Discord) }, async ({ provider }) => {
      return await provider.createAuthorizationURL(state, ['email'])
    })
...
const checkAuthTokens = async (tokens: Partial<AuthTokens>, authProvider: AuthProvider) => {
  let accessToken: string | undefined = typeof tokens.accessToken === 'function' 
    ? tokens.accessToken()
    : tokens.accessToken
  ...
  if ('idToken' in tokens && tokens.idToken && authProvider instanceof Apple) {
    idTokenClaims = getAppleClaims(tokens.idToken())
  }
  if ('refreshToken' in tokens) {
    refreshToken = typeof tokens.refreshToken === 'function' 
      ? tokens.refreshToken()
      : tokens.refreshToken
  }
    ...
    if (!accessTokenExpiresAt || !isWithinExpirationDate(accessTokenExpiresAt)) {
      if (refreshToken && 'refreshAccessToken' in authProvider) {
        ...
        if (refreshedTokens && 'idToken' in refreshedTokens) {
          idTokenClaims = getAppleClaims(refreshedTokens.idToken())
        }
      }
    }
    if (!accessTokenExpiresAt || !isWithinExpirationDate(accessTokenExpiresAt)) {
...
export async function getDiscordUser({
  ...
  ).json()) as { email: string; id: string }
...
export async function getGitHubUser({
  ...
  ).json()) as { primary: boolean; email: string }[]
